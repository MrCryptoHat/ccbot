...
class TestPrecedesInteractivePrompt:
    """The text block right before an AskUserQuestion is flagged so
    handle_new_message can upgrade the pane-surfaced copy instead of
    duplicating it. Claude Code may write the prose, thinking, and the
    AskUserQuestion as separate assistant JSONL messages — the flag has to
    look across messages, not just within one."""

    def test_separate_messages_text_then_thinking_then_ask(
        self,
        make_jsonl_entry,
        make_text_block,
        make_thinking_block,
        ...
        entries = [
            make_jsonl_entry("assistant", [make_thinking_block("hm")]),
            make_jsonl_entry("assistant", [make_text_block("Here is what I found.")]),
            make_jsonl_entry(
                "assistant",
                [make_tool_use_block("t1", "AskUserQuestion", {"questions": []})],
            ),
        ]
        result, _ = TranscriptParser.parse_entries(entries)
        text_entries = [e for e in result if e.content_type == "text"]
        assert len(text_entries) == 1
        assert text_entries[0].precedes_interactive_prompt is True

    def test_same_message_text_and_ask(
    ...
    def test_no_prose_when_user_message_precedes_ask(
        self, make_jsonl_entry, make_text_block, make_tool_use_block
    ):
        entries = [
            make_jsonl_entry("user", [make_text_block("спроси меня")]),
            make_jsonl_entry(
                "assistant",
                [make_tool_use_block("t1", "AskUserQuestion", {"questions": []})],
            ),
        ...
        result, _ = TranscriptParser.parse_entries(entries)
        # No assistant text entry at all; nothing to flag, nothing crashes.
        assert all(not e.precedes_interactive_prompt for e in result)

    def test_unrelated_text_not_flagged(
        self, make_jsonl_entry, make_text_block, make_tool_use_block
    ):
        entries = [
            make_jsonl_entry("assistant", [make_text_block("Just a normal reply.")]),
            make_jsonl_entry("user", [make_text_block("ok")]),
            make_jsonl_entry("assistant", [make_text_block("Another reply.")]),
        ]
        result, _ = TranscriptParser.parse_entries(entries)
        assert all(not e.precedes_interactive_prompt for e in result)

    def test_only_last_text_before_ask_is_flagged(
        self,
        make_jsonl_entry,
        make_text_block,
        make_tool_use_block,
        ...
        entries = [
            make_jsonl_entry("assistant", [make_text_block("First, let me check.")]),
            make_jsonl_entry(
                "assistant", [make_tool_use_block("t1", "Bash", {"command": "ls"})]
            ),
            make_jsonl_entry("user", [make_tool_result_block("t1", "a\nb")]),
            make_jsonl_entry(
                "assistant", [make_text_block("Based on that — which way?")]
            ),
            make_jsonl_entry(
                "assistant",
                [make_tool_use_block("t2", "AskUserQuestion", {"questions": []})],
            ),
        ]
        result, _ = TranscriptParser.parse_entries(entries)
        flagged = [
            e
            for e in result
            if e.content_type == "text" and e.precedes_interactive_prompt
        ]
        unflagged = [
            e
            for e in result
            if e.content_type == "text" and not e.precedes_interactive_prompt
        ]
        assert [e.text for e in flagged] == ["Based on that — which way?"]
        assert [e.text for e in unflagged] == ["First, let me check."]
