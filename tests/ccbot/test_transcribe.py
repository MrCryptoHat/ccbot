"""Unit tests for transcribe — voice-to-text via Deepgram (primary) / OpenAI (fallback).

The tests below exercise the OpenAI fallback path because that's what the
fixture forces via ``deepgram_api_key = None``. The Deepgram path (JSON
shape ``{"results": {"channels": [{"alternatives": [{"transcript": ...}]}]}}``)
is trusted to the real service and its end-to-end integration; unit-testing
the fallback path covers the only branch this module routes differently.
"""
...
@pytest.fixture
def mock_config():
    """Patch config to force the OpenAI branch.

    Unsetting ``deepgram_api_key`` routes ``transcribe_voice`` through
    ``_transcribe_openai`` — the path these tests are written against.
    Without this, the real DEEPGRAM_API_KEY from .env would route into
    ``_transcribe_deepgram`` and the OpenAI-shaped mocks below would
    mismatch the Deepgram JSON response shape.
    """
