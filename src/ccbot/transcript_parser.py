...
class ParsedEntry:
    ...
    # True on the assistant text block that sits in the same message as an
    # AskUserQuestion tool_use. interactive_ui surfaces that prose from the pane
    # before the user answers; the flag lets handle_new_message upgrade that
    # already-posted message in place instead of sending the JSONL copy as a
...
class TranscriptParser:
    ...
    @staticmethod
    def _ask_question_prose_indices(entries: list[dict]) -> set[int]:
        """Indices into ``entries`` of assistant messages whose text block is the
        prose immediately preceding an AskUserQuestion call.

        Claude Code may write the prose, any extended-thinking, and the
        AskUserQuestion tool_use as *separate* assistant JSONL messages — so this
        scans across messages: from each AskUserQuestion it walks back to the
        nearest assistant message that carries a text block, skipping
        thinking-only ones, and stopping at a non-assistant message (then there
        was no prose before the question). The whole turn is held out of JSONL
        until the user answers, so these entries arrive in the same poll batch
        as the AskUserQuestion entry — interactive_ui has already surfaced this
        prose from the pane, and handle_new_message uses this to upgrade that
        message in place instead of sending the JSONL copy as a duplicate.
        """
        indices: set[int] = set()
        ...
        for k, data in enumerate(entries):
            if not any(
                ...
                for b in _content(data)
            ):
                continue
            # Walk back from k itself (the prose may be a text block in the same
            # assistant message as the AskUserQuestion call), then earlier ones.
            for j in range(k, -1, -1):
                if entries[j].get("type") != "assistant":
                    break  # user message / tool_result — no prose precedes the AUQ
                blocks = _content(entries[j])
                if any(isinstance(b, dict) and b.get("type") == "text" for b in blocks):
                    indices.add(j)
                    break
                # thinking-only (or tool_use-only) assistant message → keep going
        return indices
    ...
    def parse_entries(
        ...
        # Entry indices whose text block is the prose right before an
        # AskUserQuestion call — interactive_ui already surfaced it from the
        # pane, so handle_new_message should upgrade that message rather than
        # send the JSONL copy as a duplicate.
        ask_prose_indices = cls._ask_question_prose_indices(entries)
        ...
        for idx, data in enumerate(entries):
            ...
            if msg_type == "assistant":
                ...
                has_text = False
                precedes_ask = idx in ask_prose_indices
