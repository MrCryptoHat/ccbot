...
class MonitorState:
    ...
    def save(self) -> None:
        """Persist state via the shared off-loop writer (atomic + fsync).

        save_if_dirty runs on every monitor tick — an inline fsync would
        stall the event loop each poll. The snapshot dict is fully
        materialized here (to_dict per session), so the writer thread
        serializes frozen data; write errors are logged by the writer.
        """
        from .utils import schedule_async_json_write
        ...
        schedule_async_json_write(self.state_file, data)
        self._dirty = False
