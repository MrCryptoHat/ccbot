...
async def provision_worktree_agent(
    ...
    """Create the worktree and its topic. Returns (ok, message).

    Stops short of starting an agent: the topic is bound to the worktree
    DIRECTORY (``thread_directory_memory``) and the first message there opens
    the session picker, exactly like any other topic. Inheriting the parent's
    CLI was the old behaviour and it removed the one choice a parallel agent
    exists for — running the same files under a different model (operator
    request 2026-09-04).
...
async def _handle_wt_new(
    ...
    """🌳 New agent in the project — resolve the repo, then ask for a task name.

    No runtime step: the agent is picked in the new topic, on its first
    message, together with the session.
    """
