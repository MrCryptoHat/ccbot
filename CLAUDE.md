...
- **1 Topic = 1 Binding = 1 Session** — every internal route is keyed by the binding value. Two shapes share that string slot:
  ...
  - **Docker** (optional, `DOCKER_AGENTS_ENABLED=true`): `docker:<agent>` — Claude Code inside a container, driven via `docker exec <ctn> tmux send-keys/capture-pane -t claude`. `docker:<agent>/<slug>` is a **sibling agent** (➕): a second Claude in the SAME container, own tmux session `claude-<slug>`, shared workspace/claude-home — `.claude/rules/sibling-agents.md`.
  - `session_manager.resolve_binding(user, thread) → ("tmux","@12") | ("docker","assistant") | None` is the transport-selection entry point; `send_to_window` branches internally so most call sites stay transport-agnostic. Docker call sites resolve container + in-container tmux session through **`session_manager.resolve_docker_target(value)`**, never `config.get_docker_agent(value[7:])` — that shape is blind to siblings. **Add new transport-agnostic operations as `SessionManager` wrappers** (`send_keys`/`capture_pane`/`kill_agent`/…), not as per-call-site `if docker:` branches.
...
- **User-facing strings live in the `i18n.py` catalog — a new one goes through `tr("ns.key")` + a catalog entry, NEVER a hardcoded literal.** The UI is English only (the ru half was dropped 2026-09-02: one language to keep in sync); a missing key renders as the key, so a gap degrades, never crashes. Copy is written for a non-technical reader (plain words over jargon: «End session», not «Kill»). NOT in the catalog (leave as-is): agent-directed text (voice protocol), logs, and Claude Code's own pane markers matched for parsing. Menu-button labels (`filters.Regex` dispatcher) and `BotCommand` descriptions (`build_bot_commands()`) both read the catalog, so a label change is a catalog edit only, no dispatcher/description drift.
...
- **`is_interactive_ui` has the same trap and the same shape of answer: a widget must OWN the bottom of the pane** — an agent CLI replaces its input box with a live modal, so markers found above a drawn input box (`_live_region_start`) are a transcript *quoting* a widget, not a widget. Without that, an answer of ours quoting a menu (cursor glyph and all — this repo's topic is full of them) pinned the topic in interactive mode and bounced every following message with «агент ждёт ответа в диалоге» while no dialog existed. A new `UIPattern` whose markers can occur in prose — above all a cursor on a numbered option — needs `tail_only` for the same reason.
- **`(send file: /path)` is the security perimeter against a compromised agent** — for docker bindings `session_manager.resolve_agent_file_path` accepts **only** `/workspace/*` (→ `agent.workspace_host_path`); any other container prefix (`/auth/...`, `/root/...`) or `..` traversal is rejected. Tmux bindings pass through verbatim (legacy host paths). Don't loosen this — bind-mounted secrets like `/auth/.credentials.json` sit one path away. The marker is ccbot's OWN protocol (nothing in Claude Code hints at it), so the SessionStart hook hands every session `hook._AGENT_BRIEF` as context and a container hook carries its own copy (`docs/docker-agents.md`) — change the syntax and both change with it.
...
- `.claude/rules/sibling-agents.md` — another agent on the SAME files (➕ panel button): the in-container contract (`AGENT_NAME`, pinned `--session-id`), why docker rows outlive session_map absence, exact tmux targets
