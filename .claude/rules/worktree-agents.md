...
- **Reuse `create_window` + `bind_thread` directly, NOT `_auto_bind_to_directory`** — the latter renames the topic to the window's slug on dedup, clobbering the curated `🌳 <repo> · <task>` name.
