...
- **🌳 provisions the worktree, NOT an agent** — the flow is 🌳 → task name → `provision_worktree_agent` (git worktree + branch + topic + `worktree_meta` + `record_thread_directory(path)`), and it stops there: no window, no binding, and deliberately **no runtime recorded**. The first message in the new topic hits the normal memory-rebind and opens the session picker, so the agent (and any session) is chosen there. The old flow asked for the runtime up front (`CB_WT_RUNTIME`, removed 2026-09-04) and, for siblings, inherited the parent's CLI — which took away the one choice a parallel agent exists for: the same files under a different model. NOTE: codex's own panel hides 🌳 (`panel_actions` has no `worktree`), so you fork worktrees FROM claude topics; the new agent can then be any installed CLI.
