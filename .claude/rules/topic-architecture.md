...
2. **Tmux directory** — `_find_matching_dir_for_topic` checks `~/projects/<name>` then `~/agents/<name>` (projects wins on duplicate; underscore-prefixed and dotfile names are skipped — infra/hidden dirs). On hit: if Claude sessions already exist in that dir, surface the session picker right away (state shape reuses what `_handle_session_{select,new,cancel}` already consume, so no new callback wiring); otherwise create a fresh tmux window and bind silently. On `create_window` collision (`ccbot` window already taken → `ccbot-2`), `edit_forum_topic` renames the topic to match the dedup'd window name.
