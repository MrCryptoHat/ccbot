...
- `preview.py` — preview-server registry helpers (`~/.local/state/preview/registry.json`), consumed by `commands` and `live_board`.
...
Handlers (`handlers/`):
- `commands.py` — `/status`, `/commands` (alias `/screenshot`), `/esc`, `/kill`, `/restart`, `/voice`, `/menu`, `/remount`/`/mount`/`/umount`, `/bind`; `topic_created_handler` + `_try_auto_bind_topic` (auto-bind by topic name on `forum_topic_created` → docker agent OR `~/projects/<name>` / `~/agents/<name>`; only fires for topics created while ccbot is online — renamed/legacy topics fall back to the dir-browser flow, see `topic-architecture.md` for the deliberate-non-fix); `MENU_KEYBOARD` (persistent 2-button ReplyKeyboard 🖥️ Сервер / 👾 Агент) + `menu_button_dispatcher` (taps arrive as plain text, routed to the underlying slash commands); `_build_commands_keyboard(tab="nav"|"act")` — the unified agent-panel keyboard (pane photo + two tabs: Nav = raw key presses for the TUI `⎋ ^C ^B` (`^B` = Claude Code "send the running task to the background") / `/ ↑ ↓ ⏎`; Act = session controls 🎯 Режим (Shift+Tab cycle: normal → auto-accept → plan) / 🤖 Model / 🔌 MCP, 📊 Context / 🗜 Сжать / 🧹 Clear, ⏪ Resume / 🔄 Restart / 🆕 New / ⏹ Exit (Restart relaunches Claude with `--resume <current>`; New relaunches without `--resume` so a fresh session_id starts and the old one stays in the /resume picker); common 📸 Обновить). Tab switch = `editMessageReplyMarkup`-only (no upload); refresh/post-action repaints = `editMessageMedia`. Destructive actions (Clear/Compact/Exit) route through a two-step `CB_CMD_CONFIRM` keyboard; Restart/New ride the same confirm — not destructive, but the confirm-button label is the only place the «same dialog» vs «from scratch» difference is spelled out. Button-colour grammar (`KeyboardButtonStyle`, from the design panel): red = irreversible loss (Clear / Exit / 🗑 delete-agent + their confirms), blue = the primary tap (📸 Обновить + the compact confirm), green = restart/new confirms only (go-forward, recoverable) — green never sits in the always-visible grid (a red-adjacent green reads as blotchy); everything else neutral. `_DESTRUCTIVE_CONFIRMS` / `_FORWARD_CONFIRMS` select the confirm colour. The first panel send reuses a cached `pane_cache` `file_id` when the pane matches a recent upload (agent idle → zero render, zero upload), else renders fresh and caches it.
...
- `browser_live.py` / `live_board.py` — docker-agent live dashboards in the shared Live topic (`config.live_dashboard_target() = (NOTIFICATIONS_CHAT_ID, LIVE_DASHBOARD_THREAD_ID)`, both env vars required or the loop exits). `browser_live` polls `<ipc_dir>/{browser-live.json,current.png}` (~1.5 s) and keeps one photo message per active docker agent (state machine: no msg_id→send; newer ts→`edit_message_media`; active=false→one `edit_message_caption` to the idle caption, photo frozen). The caption's `🔗 Tailscale · 📺 VNC` controls use HTTPS-short URLs that 302-redirect to bare `vnc://` / `tailscale://` — Telegram strips link entities for non-{http,https,tg,…} schemes, so a direct `vnc://` would be unclickable. Optional `LIVE_DASHBOARD_TAILSCALE_URL` + per-agent `DOCKER_AGENT_<NAME>_VNC_URL` supply the targets. `live_board` keeps one self-updating text message listing reachable preview servers (`preview.py` registry — slug, `preview-<slug>.<preview_domain>`, TTL, health = tmux session alive + port listening) and permanent app-hosts (`config.caddy_apps_dir/*.caddy` — `https://<host>`, backend `:<port>`, health = port up); refresh 45 s, caches the body and only edits on a real change (re-stamping the `обновлено HH:MM` header then — timestamp = last change, not "now"). `message_id`s persisted in `state.json` (`live_dashboard_message_ids`; the `__live_board__` key is reserved for the text board, browser_live only iterates real agent names). Blocking scans wrapped in `asyncio.to_thread`.
